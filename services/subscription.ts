...
export async function upgradeSubscriptionAddons(params: {
  workspaceId: string;
  addons: { name: string; quantity: number }[];
}) {
  const { workspaceId, addons } = params;
  ...
  const planAddons = subscription.plan.addons as AddonsType[];

  for (const addon of addons) {
    const def = planAddons.find((a) => a.name === addon.name);
    if (!def) throw new Error(`Invalid add-on: ${addon.name}`);
    if (def.max_quantity && addon.quantity > def.max_quantity) {
      throw new Error(`Add-on quantity exceeds limit: ${addon.name}`);
    }
  }

  if (subscription.addons_snapshot?.length) {
    await razorpay.subscriptions.removeAddon(
      subscription.razorpay_subscription_id
    );
  }

  const razorpayAddons = addons.map((addon) => {
    const def = planAddons.find((a) => a.name === addon.name)!;
    return {
      item: {
        name: def.name,
        amount: Math.round(Number(def.price_per_unit) * 100),
  ...
  await razorpay.subscriptions.createAddon(
    subscription.razorpay_subscription_id,
    razorpayAddons
  );
  ...
  await prisma.subscriptions.update({
    where: { id: subscription.id },
    data: {
      pending_addons_snapshot: addons,
      upgrade_status: 'pending_invoice'
    }
  });
